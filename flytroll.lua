...
local function createGUI()
    ...
    local title = Instance.new("TextLabel", frame); title.Text = "fly troll by abd55_55 V5.28"; title.Size = UDim2.new(0.8, 0, 0, 30); title.TextColor3 = Color3.new(1,1,1); title.BackgroundTransparency = 1
    ...
    local flyBtn = newBtn("Fly"); local lockBtn = newBtn("Lock Fly: OFF"); local noclipBtn = newBtn("Noclip: OFF"); local freezeBtn = newBtn("Freeze: OFF"); local settingsBtn = newBtn("Settings")
    
    local sGui = Instance.new("Frame", screenGui); sGui.Size = UDim2.new(0, 200, 0, 200); sGui.Visible = false; sGui.BackgroundColor3 = Color3.fromRGB(80,80,80); makeDraggable(sGui)
    local sTitle = Instance.new("TextLabel", sGui); sTitle.Text = "settings"; sTitle.Size = UDim2.new(0.7, 0, 0, 30); sTitle.BackgroundTransparency = 1
    local sX = Instance.new("TextButton", sGui); sX.Text = "X"; sX.Size = UDim2.new(0.3, 0, 0, 30); sX.Position = UDim2.new(0.7, 0, 0, 0); sX.BackgroundTransparency = 1; sX.MouseButton1Click:Connect(function() sGui.Visible = false end)
    local bgBtn = newBtn("BG Color"); bgBtn.Parent = sGui; bgBtn.Position = UDim2.new(0.05, 0, 0.2, 0)
    local btnBtn = newBtn("Btn Color"); btnBtn.Parent = sGui; btnBtn.Position = UDim2.new(0.05, 0, 0.45, 0)
    local txtBtn = newBtn("Text Color"); txtBtn.Parent = sGui; txtBtn.Position = UDim2.new(0.05, 0, 0.7, 0)
    ...
    for i=0, 255, 2 do 
        local b = Instance.new("TextButton", cScroll); b.Text = ""; b.BackgroundColor3 = Color3.fromRGB(i, i, i)
        b.MouseButton1Click:Connect(function()
            if colorTarget == "BG" then frame.BackgroundColor3 = b.BackgroundColor3; sGui.BackgroundColor3 = b.BackgroundColor3; colorGui.BackgroundColor3 = b.BackgroundColor3
            elseif colorTarget == "Btn" then for _,c in pairs(container:GetChildren()) do if c:IsA("TextButton") then c.BackgroundColor3 = b.BackgroundColor3 end end
            else for _,o in pairs(frame:GetDescendants()) do if o:IsA("TextLabel") or o:IsA("TextButton") then o.TextColor3 = b.BackgroundColor3 end end end
        end)
    end
    ...
    player.CharacterAdded:Connect(function() flying = false; locked = false; freezeBtn.Text = "Freeze: OFF"; noclipBtn.Text = "Noclip: OFF"; flyBtn.Text = "Fly"; lockBtn.Text = "Lock Fly: OFF" end)
