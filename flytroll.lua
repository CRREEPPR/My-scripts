-- Fly Troll by Abd55_55 V5.12
local player = game.Players.LocalPlayer
local guiParent = player:WaitForChild("PlayerGui")
local UIS = game:GetService("UserInputService")
local flying, locked, minimized, frozen = false, false, false, false
local speed = 50
...
local function createGUI()
    ...
    local frame = Instance.new("Frame", screenGui); frame.Size = UDim2.new(0, 180, 0, 340); frame.Position = UDim2.new(0.1, 0, 0.1, 0); frame.BackgroundColor3 = Color3.fromRGB(30, 30, 30); makeDraggable(frame)
    local title = Instance.new("TextLabel", frame); title.Text = "Fly troll V5.12"; title.Size = UDim2.new(0.8, 0, 0, 30); title.TextColor3 = Color3.new(1,1,1); title.BackgroundTransparency = 1; title.Font = Enum.Font.SourceSansBold
    ...
    local speedBox = Instance.new("TextBox", container); speedBox.Text = "50"; speedBox.PlaceholderText = "Speed (1-10000)"; speedBox.Size = UDim2.new(0.9, 0, 0, 40); speedBox.BackgroundColor3 = Color3.fromRGB(50,50,50); speedBox.TextColor3 = Color3.new(1,1,1); speedBox.LayoutOrder = 4
    ...
    -- Logic
    minBtn.MouseButton1Click:Connect(function() minimized = not minimized; container.Visible = not minimized; frame.Size = minimized and UDim2.new(0, 180, 0, 30) or UDim2.new(0, 180, 0, 340); minBtn.Text = minimized and "^" or "v" end)
    ...
    freezeBtn.MouseButton1Click:Connect(function()
        frozen = not frozen; freezeBtn.Text = frozen and "Freeze: ON" or "Freeze: OFF"
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then player.Character.HumanoidRootPart.Anchored = frozen end
    end)
    
    speedBox.FocusLost:Connect(function() speed = math.clamp(tonumber(speedBox.Text) or 50, 1, 10000) end)
    player.CharacterAdded:Connect(function() flying = false; locked = false; frozen = false; flyBtn.Text = "Fly"; lockBtn.Text = "Lock Fly: OFF"; freezeBtn.Text = "Freeze: OFF" end)
