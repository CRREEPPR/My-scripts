-- Fly Troll by Abd55_55 V3.7
local player = game.Players.LocalPlayer
local guiParent = player:WaitForChild("PlayerGui")
local UIS = game:GetService("UserInputService")
local flying, locked = false, false
local speed = 50
...
local function createGUI()
    ...
    local title = Instance.new("TextLabel", frame); title.Text = "Fly troll by abd55_55 V3.7"; title.Size = UDim2.new(1, 0, 0, 30); title.TextColor3 = Color3.new(1,1,1); title.BackgroundTransparency = 1
    ...
    local speedBox = Instance.new("TextBox", container); speedBox.PlaceholderText = "Speed (50)"; speedBox.Size = UDim2.new(0.9, 0, 0, 40); speedBox.Position = UDim2.new(0.05, 0, 0, 210)
    ...
    game:GetService("RunService").RenderStepped:Connect(function()
        if flying and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if root:FindFirstChild("FlyBV") then
                if locked then root.FlyBV.Velocity = lockedDirection * speed
                else root.FlyBG.CFrame = workspace.CurrentCamera.CFrame; root.FlyBV.Velocity = workspace.CurrentCamera.CFrame.LookVector * speed end
            end
        end
    end)
end
