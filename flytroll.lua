...
local function createGUI()
    ...
    local title = Instance.new("TextLabel", frame); title.Text = "Fly troll V5.0"; title.Size = UDim2.new(0.8, 0, 0, 30); title.TextColor3 = Color3.new(1,1,1); title.BackgroundTransparency = 1; title.Font = Enum.Font.SourceSansBold
    ...
    local container = Instance.new("ScrollingFrame", frame); container.Size = UDim2.new(1, 0, 1, -30); container.Position = UDim2.new(0, 0, 0, 30); container.BackgroundTransparency = 1; container.CanvasSize = UDim2.new(0,0,1.5,0); container.ScrollBarThickness = 4
    local layout = Instance.new("UIListLayout", container); layout.HorizontalAlignment = Enum.HorizontalAlignment.Center; layout.Padding = UDim.new(0, 8)

    local flyBtn = Instance.new("TextButton", container); flyBtn.Text = "Fly"; flyBtn.Size = UDim2.new(0.9, 0, 0, 40); flyBtn.BackgroundColor3 = Color3.fromRGB(50,50,50); flyBtn.TextColor3 = Color3.new(1,1,1)
    local lockBtn = Instance.new("TextButton", container); lockBtn.Text = "Lock Fly: OFF"; lockBtn.Size = UDim2.new(0.9, 0, 0, 40); lockBtn.BackgroundColor3 = Color3.fromRGB(50,50,50); lockBtn.TextColor3 = Color3.new(1,1,1)
    local bgBtn = Instance.new("TextButton", container); bgBtn.Text = "Background Color"; bgBtn.Size = UDim2.new(0.9, 0, 0, 40); bgBtn.BackgroundColor3 = Color3.fromRGB(50,50,50); bgBtn.TextColor3 = Color3.new(1,1,1)
    local btnBtn = Instance.new("TextButton", container); btnBtn.Text = "Buttons Color"; btnBtn.Size = UDim2.new(0.9, 0, 0, 40); btnBtn.BackgroundColor3 = Color3.fromRGB(50,50,50); btnBtn.TextColor3 = Color3.new(1,1,1)
    local txtBtn = Instance.new("TextButton", container); txtBtn.Text = "Text Color"; txtBtn.Size = UDim2.new(0.9, 0, 0, 40); txtBtn.BackgroundColor3 = Color3.fromRGB(50,50,50); txtBtn.TextColor3 = Color3.new(1,1,1)
    local speedBox = Instance.new("TextBox", container); speedBox.Text = "50"; speedBox.PlaceholderText = "Speed (1-10000)"; speedBox.Size = UDim2.new(0.9, 0, 0, 40); speedBox.BackgroundColor3 = Color3.fromRGB(50,50,50); speedBox.TextColor3 = Color3.new(1,1,1)
    ...
    local function updateAllText(col)
        for _, obj in pairs(screenGui:GetDescendants()) do
            if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                if obj ~= xBtn then obj.TextColor3 = col end
            end
        end
    end
    ...
    for _, col in pairs(colorPalette) do
        local b = Instance.new("TextButton", colorScroll); b.Text = ""; b.BackgroundColor3 = col; b.MouseButton1Click:Connect(function() 
            if colorTarget == "Background" then 
                frame.BackgroundColor3 = col; colorGui.BackgroundColor3 = col
            elseif colorTarget == "Buttons" then 
                for _, c in pairs(container:GetChildren()) do if c:IsA("TextButton") or c:IsA("TextBox") then c.BackgroundColor3 = col end end
            elseif colorTarget == "Text" then
                updateAllText(col)
            end
    ...
    player.CharacterAdded:Connect(function() flying = false; locked = false; flyBtn.Text = "Fly"; lockBtn.Text = "Lock Fly: OFF" end)
