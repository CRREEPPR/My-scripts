...
local function createGUI()
    ...
    local title = Instance.new("TextLabel", frame); title.Text = "Fly troll V5.10"; title.Size = UDim2.new(0.8, 0, 0, 30); title.TextColor3 = Color3.new(1,1,1); title.BackgroundTransparency = 1; title.Font = Enum.Font.SourceSansBold
    ...
    -- Color GUI (simplified for space)
    ...
    for _, col in pairs(colorPalette) do
        local b = Instance.new("TextButton", colorScroll); b.Text = ""; b.BackgroundColor3 = col; b.MouseButton1Click:Connect(function() 
            ...
            elseif colorTarget == "Text" then for _, obj in pairs(screenGui:GetDescendants()) do if (obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox")) and obj ~= xBtn then obj.TextColor3 = col end end end
    ...
    camBtn.MouseButton1Click:Connect(function()
        ...
        if freecam then camPos = cam.CFrame; cam.CameraType = Enum.CameraType.Scriptable
    ...
    RS.RenderStepped:Connect(function(dt)
        ...
        elseif flying and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if not r:FindFirstChild("FlyBV") then local b = Instance.new("BodyVelocity", r); b.Name="FlyBV"; b.MaxForce=Vector3.new(math.huge,math.huge,math.huge); local g = Instance.new("BodyGyro", r); g.Name="FlyBG"; g.MaxTorque=Vector3.new(math.huge,math.huge,math.huge); g.P=10000 end
            if locked then r.FlyBG.CFrame = CFrame.new(r.Position, r.Position + lockedDirection); r.FlyBV.Velocity = lockedDirection * speed
            else r.FlyBG.CFrame = cam.CFrame; r.FlyBV.Velocity = cam.CFrame.LookVector * speed end
