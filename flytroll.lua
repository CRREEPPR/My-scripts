...
local function createGUI()
    ...
    local title = Instance.new("TextLabel", frame); title.Text = "fly troll by abd55_55 V5.25"; title.Size = UDim2.new(0.8, 0, 0, 30); title.TextColor3 = Color3.new(1,1,1); title.BackgroundTransparency = 1; title.Font = Enum.Font.SourceSansBold
    ...
    local settingsBtn = newBtn("Settings")
    
    -- Settings GUI (as in 1000031833.png)
    local settingsGui = Instance.new("Frame", screenGui); settingsGui.Size = UDim2.new(0, 200, 0, 300); settingsGui.Position = UDim2.new(0.4, 0, 0.1, 0); settingsGui.Visible = false; settingsGui.BackgroundColor3 = Color3.fromRGB(150,150,150); makeDraggable(settingsGui)
    local sTitle = Instance.new("TextLabel", settingsGui); sTitle.Text = "settings"; sTitle.Size = UDim2.new(0.7, 0, 0, 30); sTitle.BackgroundTransparency = 1; sTitle.Font = Enum.Font.SourceSansBold
    local sX = Instance.new("TextButton", settingsGui); sX.Text = "X"; sX.Size = UDim2.new(0.3, 0, 0, 30); sX.Position = UDim2.new(0.7, 0, 0, 0); sX.BackgroundTransparency = 1; sX.TextColor3 = Color3.new(1,0,0); sX.MouseButton1Click:Connect(function() settingsGui.Visible = false end)
    local sContainer = Instance.new("ScrollingFrame", settingsGui); sContainer.Size = UDim2.new(1, 0, 1, -30); sContainer.Position = UDim2.new(0, 0, 0, 30); sContainer.BackgroundTransparency = 1; Instance.new("UIListLayout", sContainer).HorizontalAlignment = Enum.HorizontalAlignment.Center
    
    local speedBox = Instance.new("TextBox", sContainer); speedBox.Text = "50"; speedBox.PlaceholderText = "Fly Speed"; speedBox.Size = UDim2.new(0.9, 0, 0, 35); speedBox.BackgroundColor3 = Color3.fromRGB(60,60,60); speedBox.TextColor3 = Color3.new(1,1,1)
    local bgBtn = newBtn("Background Color"); bgBtn.Parent = sContainer; local btnBtn = newBtn("Buttons Color"); btnBtn.Parent = sContainer; local txtBtn = newBtn("Text Color"); txtBtn.Parent = sContainer

    local colorGui = Instance.new("Frame", screenGui); colorGui.Size = UDim2.new(0, 200, 0, 200); colorGui.Position = UDim2.new(0.6, 0, 0.1, 0); colorGui.Visible = false; colorGui.BackgroundColor3 = Color3.fromRGB(40, 40, 40); makeDraggable(colorGui)
    ...
    for _, col in pairs(colorPalette) do
        local b = Instance.new("TextButton", colorScroll); b.Text = ""; b.BackgroundColor3 = col; b.MouseButton1Click:Connect(function()
            if colorTarget == "Background" then frame.BackgroundColor3 = col; colorGui.BackgroundColor3 = col; settingsGui.BackgroundColor3 = col
            elseif colorTarget == "Buttons" then for _,c in pairs(container:GetChildren()) do if c:IsA("TextButton") or c:IsA("TextBox") then c.BackgroundColor3 = col end end; for _,c in pairs(sContainer:GetChildren()) do if c:IsA("TextButton") or c:IsA("TextBox") then c.BackgroundColor3 = col end end
            else for _,o in pairs(frame:GetDescendants()) do if o:IsA("TextLabel") or o:IsA("TextButton") or o:IsA("TextBox") then o.TextColor3 = col end end; for _,o in pairs(settingsGui:GetDescendants()) do if o:IsA("TextLabel") or o:IsA("TextButton") or o:IsA("TextBox") then o.TextColor3 = col end end end
        end)
    end
    ...
    settingsBtn.MouseButton1Click:Connect(function() settingsGui.Visible = true end)
    ...
    RS.RenderStepped:Connect(function()
        ...
        if flying and not frozen and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if not r:FindFirstChild("FlyBV") then local b = Instance.new("BodyVelocity", r); b.Name="FlyBV"; b.MaxForce=Vector3.new(9e9,9e9,9e9); local g = Instance.new("BodyGyro", r); g.Name="FlyBG"; g.MaxTorque=Vector3.new(9e9,9e9,9e9); g.P = 50000; g.D = 100 end
